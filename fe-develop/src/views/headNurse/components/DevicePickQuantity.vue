  ...
  <q-dialog v-model="showDialog" no-backdrop-dismiss>
    <div class="q-pt-md shadow-1 bg-white" style="max-width: 600px; width: 100%">
      ...
      <div class="q-pt-sm q-pb-md q-px-lg">
        ...
        <div class="row">
          <q-img
            :src="$utilsFilters.imagePath(tempItem.mainImagePath)"
            width="120px"
            :ratio="4 / 3"
            class="col-3"
          />
          <div class="col-9">
            <div class="column justify-between">
              <div class="q-ml-lg q-mb-md">
                <h6 class="text-weight-bold q-mb-sm">{{ tempItem.nameScientific }}</h6>
                <p class="text-body1 text-grey">{{ tempItem.name }}</p>
              </div>
        ...
        <p class="text-caption text-grey q-pt-sm">*當前庫存為{{ tempItem.totalQty }}隻</p>
      </div>
      <q-separator />
...
export default {
  ...
  computed: {
    selectVolume() {
      return this.tempItem.totalQty;
    },
    isMaxSize() {
      return this.selectQuantity >= this.selectVolume;
    },
  },
  methods: {
    checkQuantity() {
    ...
    changeQuantity(val) {
      if (this.isMaxSize) {
        this.selectQuantity = this.selectVolume;
      } else if (val <= 0) {
        this.selectQuantity = 1;
      }
    },
