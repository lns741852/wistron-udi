<template>
  ...
  <SwitchTabs ref="switchTabs" :tabList="tabList" finishWord="補申請完成" @pressFinal="init">
      ...
      <div v-show="slotProps.currentTabNumber === 1">
        <div class="q-pa-lg">
          <SurgicalOrderTable
            ...
            :surgicalStatus="applyAgainStatus"
            @dataSelected="surgicalSelected"
          />
        </div>
        <q-separator />
...
<script>
import { apiQueryPackageConf, apiApplyAgainCreate } from '@/api';
...
export default {
  ...
  methods: {
    surgicalSelected(item) {
      const [first] = item;
      this.applyInfo = first;
    },
    // 建立手術用器械申請單
    async createSurgicalApply() {
      ...
      try {
        ...
        await apiApplyAgainCreate({ surgicalApplyId: this.applyInfo.applyId, orders });
        this.$gNotifySuccess('補申請成功');
